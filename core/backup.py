...
def backup_income(storage):
    ...
    for year in all_years:
        ...
        with open(path, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow([
                "ID", "Date", "Gross Income", "Client Name", "Ref. No.", "Description",
...
def backup_expense(storage):
    ...
    for year in all_years:
        ...
        with open(path, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow([
                "ID", "Date", "Business Name", "Ref. No.", "Gross Expense", "Description",
                "WT", "ATC", "Expense Paid"
            ])
            for r in rows:
                writer.writerow([
...
def backup_all_summaries(storage, app_state):
    """
    Regenerates summary CSVs for ALL years that have income or expense data.
    """
    ensure_backup_dir()

    rows = storage.cursor.execute("""
    ...
    for r in rows:
        year = int(r["year"])
        backup_summary(storage, app_state, year)
